...
import { Box, List, ListItem, ListItemAvatar, Avatar, ListItemText, Link, CircularProgress, Alert, Typography, Chip } from '@mui/material';
import Grid from '@mui/material/Grid';
import { retryingJson } from '../../lib/retryFetch.js';
import { useDashboardSettings } from '../index.jsx';
import { formatDate } from '../../lib/dateFormat.js';

export default function RSSWidget({ url, urls, limit = 10, layout = 'list', rowMinHeight = 160, refreshSignal }) {
  ...
  useEffect(() => {
    ...
    retryingJson('/api/widget/rss-feed', {
      ...
      .then(({ data }) => { if (!active) return; setItems(data.items || []); })
...
function MsnLayout({ items, dateFmt, rowMinHeight = 160 }) {
...
function MsnCard({ item, size = 'md', sx, dateFmt }) {
  const { title, link, image, sourceTitle, pubDate, isoDate, contentSnippet, content, description, summary } = item;
  const bg = image ? `linear-gradient(180deg, rgba(0,0,0,0.2) 0%, rgba(0,0,0,0.55) 60%), url(${image})` : 'linear-gradient(180deg, rgba(0,0,0,0.15) 0%, rgba(0,0,0,0.5) 60%)';
    ...
    <Link href={link} target="_blank" rel="noreferrer" underline="none" sx={{ display: 'block', width: '100%' }}>
      <Box sx={{
        ...
        <Box sx={{ color: 'white', textShadow: '0 1px 2px rgba(0,0,0,0.8)', width: '100%' }}>
          ...
          {snippet && (
            <Typography variant={size === 'lg' ? 'body2' : 'caption'} sx={{ opacity: 0.95, mt: 0.5, display: '-webkit-box', WebkitLineClamp: clamp, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
              {snippet}
            </Typography>
          )}
          <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', mt: 0.75 }}>
            {sourceTitle && <Chip size="small" label={sourceTitle} sx={{ bgcolor: 'rgba(255,255,255,0.15)', color: 'white' }} />}
            <Typography variant="caption" sx={{ opacity: 0.85 }}>
              {formatDate(pubDate || isoDate, dateFmt)}
            </Typography>
          </Box>
        </Box>
      </Box>
