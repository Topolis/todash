...
async function fetchIcs(url: string, force: boolean): Promise<CalendarEvent[]> {
  const key = `ics:${url}`;
  ...
  for (const raw of lines) {
    ...
    if (key === 'DTSTART') {
      ev.start = parseDate(rhs);
    } else if (key === 'DTEND') {
      ev.end = parseDate(rhs);
    } else if (key === 'SUMMARY') {
      ev.title = rhs;
...
export async function fetchCalendarData(config: CalendarConfig): Promise<CalendarData> {
  ...
  const sources: CalendarSource[] =
    Array.isArray(config.sources) && config.sources.length
      ? config.sources
          ...
          .filter((s): s is CalendarSource => !!s && !!s.url)
  ...
  const results = await Promise.allSettled(
    sources.map(async (src, i) => {
      const items = await fetchIcs(src.url, config.force || false);
  ...
  const eventsMerged = results
    .filter((r): r is PromiseFulfilledResult<any[]> => r.status === 'fulfilled')
    .flatMap((r) => r.value || []);

  // Build initial status with total parsed items per source
  let sourcesStatus: SourceStatus[] = sources.map((src, i) => {
    const r = results[i];
    if (r && r.status === 'fulfilled') {
      ...
      const reason =
        r && r.status === 'rejected' ? ((r.reason as Error)?.message || String(r.reason)) : 'Unknown error';
      return {
        url: src.url,
        label: src.label || null,
        color: src.color || null,
  ...
  const filtered = eventsMerged.filter((ev: any) => {
  ...
  for (const ev of filtered) {
    if (typeof (ev as any).__src === 'number') {
      inWindowCounts[(ev as any).__src]++;
      delete (ev as any).__src;
    }
  }
